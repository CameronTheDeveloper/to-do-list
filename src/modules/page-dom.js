...
const addFolderSidebarElements = (folder, folderContentDiv, buttonClass) => {
  ...
  const sidebarTitle = addAttributes(
    folder.title,
    'class',
...
const addToDoContent = (toDoItem) => {
  ...
  const priorityLabel = addAttributes(
    'Priority - ',
    'class',
    'priority-label',
    'div',
  );
  const priorityValue = addAttributes(
    toDoItem.priority,
    'class',
    'priority',
    'div',
  );
  const descDiv = addAttributes(
